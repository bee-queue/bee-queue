...
function isClient(object) {
  ...
  return name === 'Redis' || name === 'RedisClient' || isCluster(object);
}

// check if the object is a ioredis cluster client
...
function duplicateClient(client) {
  if (isCluster(client)) {
    // ioredis doesn't have the duplicate method, which means we need to
    // manually create a new instance of it with the same configuration
    const options = Object.assign({}, client.options);
    return new ioredis.Cluster(client.startupNodes, options);
  } else {
