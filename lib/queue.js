...
Queue.prototype.destroy = function (cb) {
  const promise = this._commandable().then((client) => {
    ...
    const args = ['id', 'jobs', 'stallBlock', 'stalling', 'waiting', 'active', 'succeeded', 'failed']
 ...
 * @param {Object} page
 * @param {Number=} page.start Start of query range for waiting/active/delayed queue types
 * @param {Number=} page.end End of query range for waiting/active/delayed queue types
 * @param {Number=} page.size Number jobs to return for failed/succeeded (SET) types
 * @param {Function=} callback Called with the equivalent of the returned promise.
 * @return {Promise<Array>} Resolves to the jobs the function found.
 */
Queue.prototype.getJobs = function (type, page, cb) {
  const promise = this._commandable().then((client) => {
    ...
    switch (type) {
      case 'failed':
      case 'succeeded':
        this._scanForJobs(this.toKey(type), '0', page.size, new Set(), idsPromise.defer());
        break;
      case 'waiting':
      case 'active':
        client.lrange(this.toKey(type), page.start, page.end, idsPromise.defer());
        break;
      case 'delayed':
        client.zrange(this.toKey(type), page.start, page.end, idsPromise.defer());
        break;
      default:
...
Queue.prototype._waitForJob = function () {
  const idPromise = helpers.deferred(), start = Date.now();
  let getStart;
  /* istanbul ignore next */
  this._verbose('blockingPop');
  this.bclient.brpoplpush(this.toKey('waiting'), this.toKey('active'), 0, idPromise.defer());

  /* istanbul ignore next */
  const verboseFailure = this.settings.verbose ? (err) => {
    this._verbose('getNextJobFailed', {
      code: err.code,
      message: err.message,
      stack: err.stack,
      type: err.type,
  ...
  return idPromise.then((jobId) => {
    /* istanbul ignore next */
    this._verbose('blockingPopResponse', {
      id: jobId,
      startTime: start,
      waitTime: Date.now() - start
    });
    getStart = Date.now();
    return Job.fromId(this, jobId).then((job) => {
      /* istanbul ignore next */
      this._verbose('getNextJob', {
        jobId,
        gotJob: !!job,
        id: job ? job.id : null,
        startTime: getStart,
      ...
      // Note that the job may be null in the case that the client has removed the job before
      // processing can take place, but after the brpoplpush has returned the job id.
      return job;
    }, verboseFailure);
  }, (err) => {
    /* istanbul ignore next */
    this._verbose('blockingPopError', {
      code: err.code,
      message: err.message,
      stack: err.stack,
      type: err.type,
...
Queue.prototype._getNextJob = function () {
  // Under normal calling conditions, commandable will not reject because we will have just checked
  // paused in Queue#process.
  return this._commandable(true).then(() => this._waitForJob());
};

Queue.prototype._runJob = function (job) {
  /* istanbul ignore next */
  this._verbose('runJob', {id: job.id, timestamp: job.options.timestamp});
  ...
  /* istanbul ignore next */
  this._verbose('callingHandler', {id: job.id, timestamp: job.options.timestamp});
  ...
  const jobPromise = promise
    ...
    .then((data) => {
      ...
      // The only error that can happen here is either network- or Redis-related, or if Queue#close
      // times out while a job is processing, and the job later finishes.
      this.activeJobs.delete(jobPromise);
      throw err;
    });

  // We specifically use the value produced by then to avoid cases where the process handler returns
  // the same Promise object each invocation.
...
Queue.prototype._preventStall = function (jobId) {
  ...
  /* istanbul ignore next */
  this._verbose('preventStall', {id: jobId});
  this.client.srem(this.toKey('stalling'), jobId, cb);
  /* istanbul ignore next: these errors are exclusively redis or network errors */
...
Queue.prototype.process = function (concurrency, handler) {
  ...
  const jobTick = () => {
    ...
    this._getNextJob().then((job) => {
      // We're shutting down.
      if (this.paused) {
        // This job will get picked up later as a stalled job if we happen to get here. We can't
        // easily process this job because at this point Queue#close has already captured the
        // activeJobs set in a Promise.all call, and we'd prefer to delay a job than half-process
        // it.
        this.queued -= 1;
        return;
      }
      ...
      /* istanbul ignore next */
      this._verbose('gotJob', {id: job && job.id, timestamp: job && job.options.timestamp});
      ...
      if (!job) {
        // Per comment in Queue#_waitForJob, this branch is possible when the job is removed before
        // processing can take place, but after being initially acquired.
        setImmediate(jobTick);
        return;
      }
...
Queue.prototype._doStalledJobCheck = function () {
  return this._evalScript('checkStalledJobs', 4,
    this.toKey('stallBlock'), this.toKey('stalling'), this.toKey('waiting'), this.toKey('active'),
  ...
  if (interval && !this.checkTimer) {
    promise.then(() => {
      ...
      this.checkTimer = setTimeout(() => {
        // The checkTimer is removed when Queue#close is called, so we don't need to check for it
        // here.
        this.checkTimer = null;
        const postStalled = this._checkStalledJobs(interval, cb);
        // If it's not the first call, and a callback is not defined, then we must emit errors to
        // avoid unnecessary unhandled rejections.
        /* istanbul ignore next: these errors are limited to redis and connection errors */
...
/**
 * Emit a verbose event, for logging.
 */
Queue.prototype._verbose = function (name, details) {
  /* istanbul ignore if */
