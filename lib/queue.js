...
class Queue extends Emitter {
  ...
  _getJobsByIds(ids) {
    ...
    return this._commandable().then((client) => {
      ...
      const count = idsArray.length, jobs = new Array(count);
      ...
      for (let i = 0; i < count; ++i) {
        ...
        if (jobData) {
          jobs[realCount++] = Job.fromData(this, idsArray[i], jobData);
        }
      }
      jobs.length = realCount;
      return jobs;
    });
  }
  ...
  getJobs(type, page, cb) {
    const promise = this._commandable().then((client) => {
      ...
      return idsPromise;
    }).then((ids) => this._getJobsByIds(ids));
