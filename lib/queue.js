...
Queue.prototype.getNextJob = function (cb) {
  const promise = this._commandable(true).then(() => {
...
Queue.prototype.process = function (concurrency, handler) {
  ...
  const jobTick = () => {
    ...
    this.getNextJob().then((job) => {
      ...
      this._verbose('gotJob', {id: job.id, timestamp: job.options.timestamp});
      ...
      return this.runJob(job).catch((err) => {
...
Queue.prototype._checkStalledJobs = function (isFirstCall, interval, cb) {
  const promise = this._doStalledJobCheck();

  if (cb) helpers.asCallback(promise, cb);
  else if (!isFirstCall) {
    // If it's not the first call, and a callback is not defined, then we must emit errors to avoid
    // unnecessary unhandled rejections.
  ...
  if (interval && !this.checkTimer) {
    promise.then(() => {
      ...
      this.checkTimer = setTimeout(() => {
        this.checkTimer = null;
        this._checkStalledJobs(false, interval, cb);
      }, interval);
    });
  }
...
Queue.prototype.checkStalledJobs = function (interval, cb) {
  ...
  return this._checkStalledJobs(true, interval, cb);
};
