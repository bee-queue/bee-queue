...
Queue.prototype._scanForJobs = function (key, cursor, size, set, cb) {
  const count = Math.min(size, this.settings.redisScanCount);
  this.client.sscan(key, cursor, 'COUNT', count, (err, results) => {
    /* istanbul ignore if */
    if (err) {
      return cb(err);
    }
