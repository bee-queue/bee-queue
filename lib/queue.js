...
function Queue(name, settings) {
  ...
  var makeClient = function (name) {
    this[name] = redis.createClient.apply(redis, this.connParams);
    this[name].on('error', this.emit.bind(this, 'error'));
    this[name].select(this.settings.db, reportReady);
  }.bind(this);
...
/*
todo test and document different signatures
add(data)
add(data, options)
add(data, cb)
add(data, options, cb)
...
Queue.prototype.runJob = function (job, cb) {
  ...
  var handleOutcome = function (err, data) {
    ...
    // todo refactor most of here down into a separate function

    var status = err ? 'failed' : 'succeeded';

    var multi = self.client.multi()
      .lrem(self.toKey('active'), 0, job.id)
      .srem(self.toKey('stalling'), job.id);
    ...
    if (status === 'failed') {
      if (job.options.retries > 0) {
        ...
        jobEvent.data = job.options.retries; // todo send more than just this? send the error?
        multi.hset(self.toKey('jobs'), job.id, job.toData())
             .lpush(self.toKey('waiting'), job.id);
      } else {
        multi.sadd(self.toKey('failed'), job.id);
      }
    } else {
      if (self.settings.removeOnSuccess) {
        multi.hdel(self.toKey('jobs'), job.id);
      } else {
        multi.sadd(self.toKey('succeeded'), job.id);
      }
    }

    if (self.settings.sendEvents) {
      multi.publish(self.toKey('events'), JSON.stringify(jobEvent));
    }
