...
class Queue extends Emitter {
  ...
  _finishJob(err, data, job) {
    ...
    if (err) {
      ...
      if (delay < 0) {
        job.status = 'failed';
        removeJob = this.settings.removeOnFailure;
      } else {
        job.options.retries -= 1;
        job.status = 'retrying';
        jobEvent.event = 'retrying';
        if (delay !== 0) {
          delayedTime = Date.now() + delay;
        }
      }
    } else {
      job.status = 'succeeded';
      removeJob = this.settings.removeOnSuccess;
    }
    ...
    const promise = this._evalScript(
      ...
      this.toKey('failed'),
      this.toKey('delayed'),
      this.toKey('earlierDelayed'),
      this.toKey('succeeded'),
      this.toKey('events'),
      this.toKey('jobs'),
      job.id,
      removeJob ? '' : job.toData(),
      job.status,
      delayedTime || '',
      this.settings.sendEvents ? JSON.stringify(jobEvent) : ''
    );
