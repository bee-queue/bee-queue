...
function Queue(name, settings) {
  ...
  var numClients = 1 + this.settings.isWorker + this.settings.getEvents;
  var reportReady = barrier(1 + numClients, this.emit.bind(this, 'ready'));

  // todo DRY up client creation
  ...
  if (this.settings.getEvents) {
    this.jobs = {};
    this.eclient = redis.createClient.apply(redis, this.connParams);
    this.eclient.on('error', this.emit.bind(this, 'error'));
    this.eclient.select(this.settings.db, reportReady);
    this.eclient.on('message', this.onMessage.bind(this));
    this.eclient.subscribe(this.toKey('events'));
    // client.on('subscribe', function () {
    //   // todo maybe make this happen before reportReady also? or do callbackQueue like kue
...
Queue.prototype.onMessage = function (channel, msg) {
  msg = JSON.parse(msg);
  if (this.jobs[msg.id]) {
    if (msg.event === 'failed') {
    ...
    this.jobs[msg.id].emit(msg.event, msg.data);

    if (msg.event === 'succeeded' || msg.event === 'failed') {
...
Queue.prototype.runJob = function (job, cb) {
  ...
  var handleOutcome = function (err, data) {
    ...
    job.moveToSet(status, function (errMove) {
      ...
      self.client.publish(self.toKey('events'), JSON.stringify({
        id: job.id,
        event: status,
        data: data
      }));
