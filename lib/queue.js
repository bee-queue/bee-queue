...
class Queue extends Emitter {
    ...
    if (this.settings.getEvents || this.settings.activateDelayedJobs) {
      eventsPromise = makeClient('eclient', true).then(() => {
        ...
        return Promise.all(channels.map((channel) => {
          const promise = this.eclient.subscribe(channel);
          return promise;
        }));
      });
    }
  ...
  _addJobsByIds(jobs, ids) {
    ...
    return this._commandable().then((client) => {
      const commandArgs = [this.toKey('jobs')].concat(ids);
      const got = client.hmget(commandArgs);
      return got;
    }).then((dataArray) => {
  ...
  getJobs(type, page) {
    ...
    const promise = this._commandable().then((client) => {
      ...
      switch (type) {
        case 'failed':
        case 'succeeded':
          idsPromise = this._scanForJobs(key, '0', page.size, new Set());
          break;
        case 'waiting':
        case 'active':
          idsPromise = client.lrange(key, page.start, page.end);
          break;
        case 'delayed':
          idsPromise = client.zrange(key, page.start, page.end);
          break;
        default:
  ...
  _waitForJob() {
    const idPromise = this.bclient.brpoplpush(
      this.toKey('waiting'),
      this.toKey('active'),
      0
    );
  ...
  process(concurrency, handler) {
    ...
    // todo make sure this is how we want to kick off the jobtickin
    this.checkStalledJobs().then(jobTick);
    ...
    if (interval && !this.checkTimer) {
      promise.then(() => {
        ...
        this.checkTimer = setTimeout(() => {
          ...
          this.checkTimer = null;
          this._checkStalledJobs(interval);
          // no more cb, no more catch-and-emit-if-cb
        }, interval);
      });
    }
   ...
   *   function found.
   */
  checkStalledJobs(interval) {
    if (!Number.isSafeInteger(interval)) {
      interval = null;
    }
    return this._checkStalledJobs(interval);
  }
  ...
  _evalScript(name) {
    ...
    return this._commandable().then((client) => {
      ...
      const promise = client.evalsha(args);
      return promise;
    });
  }
}
