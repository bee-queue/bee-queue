...
function Queue(name, settings) {
  ...
  this.settings = {
    ...
    serializeError: settings.serializeError || function (err) {
    ...
    deserializeError: settings.deserializeError || function (err) {
      if (err.name && err.message && err.stack) { // Check for serialized error by duck typing
        var _err = err;
...
Queue.prototype.onMessage = function (channel, message) {
  ...
  if (message.event === 'failed' || message.event === 'retrying') {
    message.data = this.settings.deserializeError(message.data);
  }
...
Queue.prototype.finishJob = function (err, data, job, cb) {
  ...
  var jobEvent = {
    id: job.id,
    event: status,
    data: err ? this.settings.serializeError(err) : data
  };
