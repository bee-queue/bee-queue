...
Queue.prototype.runJob = function (job, cb) {
  if (this.settings.verbose) {
    this.emit('runJob', {id: job.id, timestamp: job.timestamp});
  }
  ...
  if (this.settings.verbose) {
    this.emit('callingHandler', {id: job.id, timestamp: job.timestamp});
  }
...
Queue.prototype.process = function (concurrency, handler) {
  ...
  const jobTick = () => {
    ...
    this.getNextJob((getErr, job) => {
      ...
      if (this.settings.verbose) {
        this.emit('gotJob', {id: job.id, timestamp: job.timestamp});
      }
