  ...
  var handleOutcome = function (err, data) {
    ...
    if (err) {
      job.moveToSet('failed', function (errMove) {
        ...
        // todo make sure we remove from stalling set
        // job.releaseLock();
        if (errMove) {
          return cb(errMove);
        }
...
Queue.prototype.resetStalledJobs = function (cb) {
  // todo figure out time-idempotency situation:
  // call it 10 times within stallInterval, only run once
  this.client.evalsha(lua.shas.checkStalledJobs, 1, '', this.toKey(''), cb);
};
