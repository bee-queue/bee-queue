...
class Queue extends Emitter {
  constructor(name, settings) {
    ...
    const makeEventClient = async () => {
      ...
      return Promise.all(
        channels.map((channel) => this.eclient.subscribe(channel))
      );
    };
  ...
  close(timeout) {
    ...
    this._closed = (async () => {
      let timeoutErr = null;
      // todo can i use finally here...
      try {
        await helpers.withTimeout(
          // todo remember this .then is fine because async iife is just as bad
          this._ready.then(() => {
            ...
            return Promise.all(
              waitJobs.map((promise) => promise.catch(() => {}))
            );
          }),
          timeout
        );
  ...
  async _runJob(job) {
    ...
    const jobPromise = (async () => {
      ...
      try {
        ...
        return await this._finishJob(failureErr, successData, job);
      } finally {
