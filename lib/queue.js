...
class Queue extends Emitter {
  constructor(name, settings) {
    ...
    this.settings = {
      ...
      keyPrefix: `${settings.prefix || defaults.prefix}:`
    };

    // Check if the client being used is an ioredis cluster, which requires the
    // use of hash tags for multi commands as they need to belong to the same
    // hash slot. The only possibility of having an ioredis cluster is by
    // passing the client within the settings object
    if (redis.isCluster(this.settings.redis)) {
      this.settings.keyPrefix += `{${this.name}}:`;
    } else {
      this.settings.keyPrefix += `${this.name}:`;
    }
