...
Job.prototype.save = function (cb) {
  ...
  this.queue.client.evalsha(lua.shas.addJob, 3,
    ...
    function (err, jobId) {
      ...
      self.id = jobId;
      if (self.queue.settings.getEvents) {
...
Job.prototype.handleEvent = function (message) {
  if (message.event === 'progress') {
    this.progress = message.data;
  } else if (message.event === 'failed' || message.event === 'retrying') {
...
Job.prototype.reportProgress = function (progress, cb) {
  ...
  if (progress < 0 || progress > 100) {
    return cb(Error('Progress must be between 0 and 100'));
  }
