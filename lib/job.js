...
Job.prototype.retry = function (cb) {
  this.queue.client.multi()
    ...
    .lpush(this.queue.toKey('waiting'), this.jobId)
    .exec(cb);
};
...
Job.prototype.moveToSet = function (set, cb) {
  var multi = this.queue.client.multi()
    ...
    .srem(this.queue.toKey('stalling'), this.jobId);
  if (this.options.retries > 0 && set === 'failed') {
    this.options.retries -= 1;
    multi.hset(this.queue.toKey('jobs'), this.jobId, this.toData())
      .lpush(this.queue.toKey('waiting'), this.jobId)
      .exec(cb);
  } else if (this.queue.settings.removeOnSuccess && set === 'succeeded') {
      multi.hdel(this.queue.toKey('jobs'), this.jobId)
        .exec(cb);
  } else {
    multi.sadd(this.queue.toKey(set), this.jobId)
      .exec(cb);
  }
};
